# Package index

## Descriptive Statistics

Summarize numeric variables with comprehensive statistics and group
comparisons.

- [`easy_describe()`](https://EasyStat.github.io/EasyStat/reference/easy_describe.md)
  : Comprehensive Descriptive Statistics with Narrative
- [`easy_group_summary()`](https://EasyStat.github.io/EasyStat/reference/easy_group_summary.md)
  : Group-Wise Summary Statistics with Narrative

## Inferential Tests

Core statistical tests with automated plain-language narrative output.

- [`easy_regression()`](https://EasyStat.github.io/EasyStat/reference/easy_regression.md)
  : Run a Linear Regression with Automated Narrative Reporting
- [`easy_ttest()`](https://EasyStat.github.io/EasyStat/reference/easy_ttest.md)
  : Run an Independent-Samples t-Test with Automated Narrative Reporting
- [`easy_anova()`](https://EasyStat.github.io/EasyStat/reference/easy_anova.md)
  : Run a One-Way ANOVA with Automated Narrative Reporting
- [`easy_chisq()`](https://EasyStat.github.io/EasyStat/reference/easy_chisq.md)
  : Chi-Square Tests with Automated Narrative Reporting
- [`easy_ztest()`](https://EasyStat.github.io/EasyStat/reference/easy_ztest.md)
  : One-Sample and Two-Sample Z-Tests with Automated Narrative Reporting
- [`easy_ftest()`](https://EasyStat.github.io/EasyStat/reference/easy_ftest.md)
  : F-Test for Equality of Variances with Automated Narrative Reporting
- [`easy_correlation()`](https://EasyStat.github.io/EasyStat/reference/easy_correlation.md)
  : Correlation Analysis with Automated Narrative Reporting

## Visualization

Publication-ready ggplot2 plots with built-in interpretation summaries.

- [`easy_histogram()`](https://EasyStat.github.io/EasyStat/reference/easy_histogram.md)
  : Annotated Histogram with Normal Curve Overlay
- [`easy_boxplot()`](https://EasyStat.github.io/EasyStat/reference/easy_boxplot.md)
  : Grouped Boxplot with Outlier Detection
- [`easy_scatter()`](https://EasyStat.github.io/EasyStat/reference/easy_scatter.md)
  : Scatter Plot with Regression Line and Correlation Annotation
- [`easy_barplot()`](https://EasyStat.github.io/EasyStat/reference/easy_barplot.md)
  : Annotated Bar Chart
- [`easy_qqplot()`](https://EasyStat.github.io/EasyStat/reference/easy_qqplot.md)
  : Normal Q-Q Plot with Shapiro-Wilk Annotation
- [`easy_density()`](https://EasyStat.github.io/EasyStat/reference/easy_density.md)
  : Kernel Density Plot with Optional Group Overlay
- [`easy_correlation_heatmap()`](https://EasyStat.github.io/EasyStat/reference/easy_correlation_heatmap.md)
  : Correlation Matrix Heatmap
- [`easy_autoplot()`](https://EasyStat.github.io/EasyStat/reference/easy_autoplot.md)
  : Automatically Plot an EasyStat Result

## Theme

Consistent visual identity for all EasyStat plots.

- [`theme_easystat()`](https://EasyStat.github.io/EasyStat/reference/theme_easystat.md)
  : Apply EasyStat ggplot2 Theme

## Reporting & Export

Export results to Microsoft Word with formatted tables and narratives.

- [`export_to_word()`](https://EasyStat.github.io/EasyStat/reference/export_to_word.md)
  : Export an EasyStat Result to a Formatted Microsoft Word Document

## S3 Methods

Print and summarize easystat_result objects.

- [`print(`*`<easystat_result>`*`)`](https://EasyStat.github.io/EasyStat/reference/print.easystat_result.md)
  : Print an EasyStat Result Object
- [`summary(`*`<easystat_result>`*`)`](https://EasyStat.github.io/EasyStat/reference/summary.easystat_result.md)
  : Summarise an EasyStat Result Object

## Package

- [`EasyStat-package`](https://EasyStat.github.io/EasyStat/reference/EasyStat-package.md)
  [`EasyStat`](https://EasyStat.github.io/EasyStat/reference/EasyStat-package.md)
  : EasyStat: Automated Statistical Analysis, Visualization and
  Multi-Format Narrative Reporting
